...
app.post("/signup", async (req, res) => {
  // Kiểm tra độ dài mật khẩu và yêu cầu ít nhất một chữ hoa, một chữ thường và một số
  const passwordRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/;
  if (!passwordRegex.test(req.body.password)) {
    return res.status(400).json({
      success: false,
      errors:
        "Mật khẩu phải chứa ít nhất 8 ký tự, bao gồm ít nhất một chữ hoa, một chữ thường và một số",
    });
  }
...
app.put("/update-profile/:email", async (req, res) => {
  try {
    ...
    const phoneNumberRegex = /^0\d{9}$/; // Bắt đầu bằng số 0 và có đúng 10 số
    if (!phoneNumberRegex.test(req.body.numberPhone)) {
      return res
        .status(400)
        .json({ success: false, errors: "Số điện thoại không hợp lệ" });
    }
